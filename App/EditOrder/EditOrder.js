...
export default class EditOrder extends React.Component {
  ...
  addVoucherItemsToCart = (voucher_item) => {
    ...
    if (vouchers_to_use.length == 0) {
      ...
      for (var index in vouchers_to_use) {
        ...
        if (
          voucher_item.voucher_type == 'SkipQueue' &&
          v.voucher.voucher_type !== 'SkipQueue'
        ) {
          array.push(v);
          continue;
        }
  ...
  check_promotion_trigger = () => {
    ...
    if (
      selectedShop.all_promotions != null &&
      selectedShop.all_promotions.length > 0
    ) {
      for (var index in selectedShop.all_promotions) {
        ...
        if (currentMember != null) {
          if (promo.trigger_price != null) {
            ...
            const search_cart_promo_index = newPromo.findIndex(
              (element) => element.name == promo.cart_text
            );
          ...
          if (promo.reward_type != null && promo.reward_type == 'Discount') {
            if (promo.value_type != null && promo.value_type == 'percent') {
              ...
              if (
                promo.maximum_discount_allow != null &&
              ...
              promo.value_type != null &&
              promo.value_type == 'fixed'
            ) {
              var discount_value = promo.value ? promo.value : 0;
  ...
  onPayNowPressed = () => {
    ...
    if (currentMember != undefined) {
      ...
      if (selected_payment == 'credits') {
        if (
          parseFloat(final_price) > parseFloat(currentMember.credits).toFixed(2)
        ) {
          this.refs.toast.show(
            'Oops. Insufficient Wallet credit.\nPlease select other payment method.',
            TOAST_DURATION
            // () => {
          ...
          >
            <View style={styles.drinksView}>
              ...
              >
                ...
                >
                  {this.state.valid_vouchers != null
                    ? this.state.valid_vouchers.length
                    : '-'}{' '}
                  available
                </Text>
                <Image
              ...
              >
                <View style={styles.productDetailView}>
                  <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    />
                    <Text style={styles.productNameText}>Pick Up Time</Text>
                  </View>
                </View>
                <Text style={styles.productVoucherText}>
  ...
  renderQueueView(current_order) {
    ...
    const order_items = current_order.order_items.map((item, key) => {
      ...
      return (
        <View style={[styles.drinksView]} key={key}>
          ...
          >
            <View style={[styles.productDetailView, { paddingVertical: 10 }]}>
            ...
            {item.order_items != null && key < item.order_items.length - 1 && (
              <Image
                source={require('./../../assets/images/dotted-line.png')}
                style={styles.dottedLineImage}
              />
    ...
    return (
      <View style={styles.orderDetailView}>
        <View style={styles.drinksViewWrapper}>{order_items}</View>
      </View>
    );
    // })
